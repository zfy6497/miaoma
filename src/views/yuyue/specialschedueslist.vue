        ...
        <Row>
            <Card>
                ...
                <Tabs :animated="false"  :value='CurrentDate'   @on-click="setCurrent"> 
                    <TabPane   v-for="option in QueryDate" :label="option.Day" :name="option.Day"></TabPane>
                </Tabs>
                <Table v-if="showTable==true" :columns="shopping_col" :data="shopping_data">
                    
                </Table>
                <Form  v-if="modalList==true" :model="initCustom" :rules="initCustom" :label-width="180" style="margin-top:20px;">
    ...
    methods: {
        ...
        getDataInfo(){
            ...
            if(this.formCustom.Id&&this.formCustom.Id!=0){
                Util.post(this.geturl,{SId:this.formCustom.Id,Day:this.CurrentDate},vm,function(res,data){
                    if(res==='1'){
        ...
        saveCurrent() {
            ...
            Util.post("admin/Special/Schedues/Save",{SId:this.formCustom.Id,Day:this.CurrentDate,Single:this.single,SchedueString:JSON.stringify(data2)},vm,function (res,data) {
                if(res==='1'){
